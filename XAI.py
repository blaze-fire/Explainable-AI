...
def explain_local(df , model, target_col_name, cat_cols=None, num_cols=None,embedings=None):
    ...
    if embedings != None:
      ...
      for i in temp:
          print('{} \n predicted : {} \t true : {} '.format(df[key].iloc[i], pred[i], y_true[i]))

    for col in num_cols:
      print('Variance in column {} is :  \t {}\n\n'.format(col, df[col].var()))

    if 'linearsvc' in str(type(clf)).lower() or 'linear_model' in str(type(clf)).lower():

      if num_cols != None:
        _, X_test, _, y_test = train_test_split(df[num_cols], df[target_col_name], test_size = 0.33, random_state = 42)
        ...
        for i in temp:
            print("{} \n predicted : {}, \t true : {} \n\n".format(df[num_cols].iloc[i], pred[i], y_test.values[i]))
